...
export default async function AnalyticsPage() {
  ...
  const statusDist = ["DRAFT", "SUBMITTED", "APPROVED", "REJECTED", "LOCKED"].map(s => ({
    name: s,
    value: goals.filter((g: any) => g.status === s).length,
  }))

  // UoM distribution
  const uomDist = ["MIN", "MAX", "TIMELINE", "ZERO"].map(u => ({
    name: u,
    value: goals.filter((g: any) => g.uom === u).length,
  }))
  ...
  const quarterDist = ["Q1", "Q2", "Q3", "Q4"].map(q => ({
    quarter: q,
    checkins: checkins.filter((c: any) => c.quarter === q).length,
    onTrack: checkins.filter((c: any) => c.quarter === q && c.status === "ON_TRACK").length,
    completed: checkins.filter((c: any) => c.quarter === q && c.status === "COMPLETED").length,
  }))

  // Per-employee goal counts
  const employees = users.filter((u: any) => u.role === "EMPLOYEE")
  const empGoalData = employees.map((emp: any) => ({
    name: emp.name.split(" ")[0],
    goals: goals.filter((g: any) => g.employee.name === emp.name).length,
    approved: goals.filter((g: any) => g.employee.name === emp.name && (g.status === "APPROVED" || g.status === "LOCKED")).length,
  })).filter((e: any) => e.goals > 0)

  const totalGoals = goals.length
  const approvalRate = totalGoals > 0 ? Math.round((goals.filter((g: any) => g.status === "APPROVED" || g.status === "LOCKED").length / totalGoals) * 100) : 0
